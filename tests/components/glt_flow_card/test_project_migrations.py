...
def test_migration_is_sequential_copy_on_write_and_receipted() -> None:
    ...
    assert CURRENT_PROJECT_SCHEMA_VERSION == 2
    assert source == original
    assert result["candidate"] is not source
    assert [(step["from"], step["to"]) for step in result["receipt"]["steps"]] == [(0, 1), (1, 2)]
...
def test_future_and_invalid_sources_fail_closed() -> None:
    with pytest.raises(ValueError, match="unsupported project schema version 3"):
        migrate_project_document({**_current_project(), "schema_version": 3})
