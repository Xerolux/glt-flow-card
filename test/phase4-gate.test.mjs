/**
 * The Phase-4 gate must fail closed.
 *
...
function healthyPlan() {
  const commands = [
    ...
    { command: "npm run test:phase4:release", id: "T4-14", name: "Exact artifacts", owner: true },
  ];
  const mapping = (evidence) => ({ evidence, text: "seeded" });
  return {
    assumptions: { A1: mapping(["T4-14"]) },
    commands,
    plans: { "02-17": mapping(["T4-14"]) },
    requirements: { "SEC-01": mapping(["F2-02", "T4-14"]) },
    roadmap: { "RC2-1": mapping(["F2-02"]) },
    threats: { "T4-14": mapping(["T4-14"]) },
  };
}
...
function healthyResults() {
  return {
    ...
    "T4-14": {
      command: "npm run test:phase4:release",
      exit_code: 0,
...
test("a healthy graph reaches the T4-14 leaf exactly once", () => {
...
test("a missing leaf leaves T4-14 unowned", () => {
...
test("complete current evidence verifies", () => {
  assert.deepEqual(validatePhase4Evidence(healthyPlan(), healthyResults()), { verified: true });
});

test("missing evidence fails closed", () => {
  const results = healthyResults();
  delete results["T4-14"];
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /missing command result: T4-14/);
});

test("a failed command fails closed", () => {
  const results = healthyResults();
  results["T4-14"] = { ...results["T4-14"], exit_code: 1, passed: false };
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /command failed: T4-14/);
});

test("a skipped command fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], skipped: true };
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /skipped: F2-02/);
});

test("a run that asserted nothing fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], test_count: 0 };
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /zero tests recorded: F2-02/);
});

test("evidence recorded for a different command is stale", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], command: "npm test -- --only=fast" };
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /stale command result: F2-02/);
});

test("evidence with no output hash fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], output_sha256: "not-a-hash" };
  assert.throws(() => validatePhase4Evidence(healthyPlan(), results), /output hash missing: F2-02/);
});
...
test("a mapping that points at evidence nobody runs fails closed", () => {
  ...
  assert.throws(() => validatePhase4Evidence(plan, healthyResults()), /references missing evidence T2-99/);
});
...
test("an unmapped requirement, plan, roadmap truth or assumption fails closed", () => {
  for (const kind of ["assumptions", "plans", "requirements", "roadmap"]) {
    ...
    assert.throws(() => validatePhase4Evidence(plan, healthyResults()), new RegExp(`${id} .* is unmapped`), kind);
  }
});
...
test("a command nothing maps to fails closed", () => {
  ...
  assert.throws(() => validatePhase4Evidence(plan, results), /command evidence is unmapped: F2-99/);
});
...
test("the repository plan binds every threat, plan, requirement and assumption", async () => {
  const plan = await loadPhase4Plan({});
  ...
  assert.deepEqual(Object.keys(plan.requirements).sort(), ["NAV-01", "OPS-02"]);
  assert.equal(Object.keys(plan.roadmap).length, 5);
  assert.deepEqual(Object.keys(plan.assumptions).sort(), [
    "COUNTS_AFTER_FILTER", "LINK_REAUTHORIZED", "NO_DISPATCH_TARGET",
    "NO_INTERPOLATION", "ONLY_READBACK_IS_SUCCESS", "SERVER_COMPOSES",
  ]);
