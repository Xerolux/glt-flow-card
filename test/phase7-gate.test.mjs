/**
 * The Phase-7 gate must fail closed.
 *
...
function healthyPlan() {
  const commands = [
    { command: "npm test", id: "F7-02", name: "Node regression suites", owner: false },
    { command: `npm run ${LEAF_SCRIPT}`, id: "T7-21", name: "Exact artifacts", owner: true },
  ];
  const mapping = (evidence) => ({ evidence, text: "seeded" });
  return {
    assumptions: { ONE_EVALUATOR: mapping(["T7-21"]) },
    commands,
    plans: { "07-20": mapping(["T7-21"]) },
    requirements: { "ALM-01": mapping(["F7-02", "T7-21"]) },
    roadmap: { "RC6-1": mapping(["F7-02"]) },
    threats: { "T7-21": mapping(["T7-21"]) },
  };
}
...
test("complete current evidence verifies", () => {
  assert.deepEqual(validatePhase7Evidence(healthyPlan(), healthyResults()), { verified: true });
});

test("missing evidence fails closed", () => {
  const results = healthyResults();
  delete results["T7-21"];
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /missing command result: T7-21/);
});

test("a failed command fails closed", () => {
  const results = healthyResults();
  results["T7-21"] = { ...results["T7-21"], exit_code: 1, passed: false };
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /command failed: T7-21/);
});

test("a skipped command fails closed", () => {
  const results = healthyResults();
  results["F7-02"] = { ...results["F7-02"], skipped: true };
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /skipped: F7-02/);
});

test("a run that asserted nothing fails closed", () => {
  const results = healthyResults();
  results["F7-02"] = { ...results["F7-02"], test_count: 0 };
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /zero tests recorded: F7-02/);
});

test("evidence recorded for a different command is stale", () => {
  const results = healthyResults();
  results["F7-02"] = { ...results["F7-02"], command: "npm test -- --only=fast" };
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /stale command result: F7-02/);
});

test("evidence with no output hash fails closed", () => {
  const results = healthyResults();
  results["F7-02"] = { ...results["F7-02"], output_sha256: "not-a-hash" };
  assert.throws(() => validatePhase7Evidence(healthyPlan(), results), /output hash missing: F7-02/);
});

test("a mapping that points at evidence nobody runs fails closed", () => {
  const plan = healthyPlan();
  plan.requirements["ALM-01"].evidence = ["F7-02", "T7-99"];
  assert.throws(() => validatePhase7Evidence(plan, healthyResults()), /references missing evidence T7-99/);
});
...
test("an unmapped requirement, plan, roadmap truth or assumption fails closed", () => {
  for (const kind of ["assumptions", "plans", "requirements", "roadmap"]) {
    ...
    assert.throws(() => validatePhase7Evidence(plan, healthyResults()), new RegExp(`${id} .* is unmapped`), kind);
  }
});
...
test("a command nothing maps to fails closed", () => {
  ...
  const plan = healthyPlan();
  plan.commands.push({ command: "npm run lint", id: "F7-99", name: "Orphan", owner: false });
  const results = { ...healthyResults(), "F7-99": {
  ...
  assert.throws(() => validatePhase7Evidence(plan, results), /command evidence is unmapped: F7-99/);
});
...
test("a gap in the threat numbering is refused", () => {
  ...
  // register missing T7-07 entirely still has a plausible number of rows.
...
test("the repository plan binds every threat, plan, requirement and assumption", async () => {
  const plan = await loadPhase7Plan({});
  assert.equal(Object.keys(plan.threats).length, plan.owner_commands.length);
  assert.equal(Object.keys(plan.plans).length, 20);
  assert.deepEqual(Object.keys(plan.requirements).sort(), ["ENER-01", "HIST-01", "REPORT-01"]);
  assert.equal(Object.keys(plan.roadmap).length, 5);
  assert.deepEqual(Object.keys(plan.assumptions).sort(), [
    "ABSENT_IS_NEVER_A_VALUE", "COVERAGE_TRAVELS", "PERIODS_ARE_DELEGATED",
    "QUERIES_ARE_BOUNDED", "REACHABLE_IS_NOT_REACHED", "RUNS_ARE_REPRODUCIBLE",
    "SERVER_OWNS_HISTORY", "TWO_METER_MODELS", "UNITS_ARE_CHECKED",
  ]);
  ...
  // passes.
  assert.match(plan.roadmap["RC7-1"].text, /historian/);
