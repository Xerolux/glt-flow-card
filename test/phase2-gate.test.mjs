/**
 * The Phase-2 gate must fail closed.
 *
...
function healthyPlan() {
  const commands = [
    ...
    { command: "npm run test:phase2:release", id: "T2-16", name: "Exact artifacts", owner: true },
  ];
  const mapping = (evidence) => ({ evidence, text: "seeded" });
  return {
    assumptions: { A1: mapping(["T2-16"]) },
    commands,
    plans: { "02-17": mapping(["T2-16"]) },
    requirements: { "SEC-01": mapping(["F2-02", "T2-16"]) },
    roadmap: { "RC2-1": mapping(["F2-02"]) },
    threats: { "T2-16": mapping(["T2-16"]) },
  };
}
...
function healthyResults() {
  return {
    ...
    "T2-16": {
      command: "npm run test:phase2:release",
      exit_code: 0,
...
test("a healthy graph reaches the T2-16 leaf exactly once", () => {
...
test("a missing leaf leaves T2-16 unowned", () => {
...
test("complete current evidence verifies", () => {
  assert.deepEqual(validatePhase2Evidence(healthyPlan(), healthyResults()), { verified: true });
});

test("missing evidence fails closed", () => {
  const results = healthyResults();
  delete results["T2-16"];
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /missing command result: T2-16/);
});

test("a failed command fails closed", () => {
  const results = healthyResults();
  results["T2-16"] = { ...results["T2-16"], exit_code: 1, passed: false };
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /command failed: T2-16/);
});

test("a skipped command fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], skipped: true };
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /skipped: F2-02/);
});

test("a run that asserted nothing fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], test_count: 0 };
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /zero tests recorded: F2-02/);
});

test("evidence recorded for a different command is stale", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], command: "npm test -- --only=fast" };
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /stale command result: F2-02/);
});

test("evidence with no output hash fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], output_sha256: "not-a-hash" };
  assert.throws(() => validatePhase2Evidence(healthyPlan(), results), /output hash missing: F2-02/);
});
...
test("a mapping that points at evidence nobody runs fails closed", () => {
  ...
  assert.throws(() => validatePhase2Evidence(plan, healthyResults()), /references missing evidence T2-99/);
});
...
test("an unmapped requirement, plan, roadmap truth or assumption fails closed", () => {
  for (const kind of ["assumptions", "plans", "requirements", "roadmap"]) {
    ...
    assert.throws(() => validatePhase2Evidence(plan, healthyResults()), new RegExp(`${id} .* is unmapped`), kind);
  }
});
...
test("a command nothing maps to fails closed", () => {
  ...
  assert.throws(() => validatePhase2Evidence(plan, results), /command evidence is unmapped: F2-99/);
});
...
test("the repository plan binds every threat, plan, requirement and assumption", async () => {
  const plan = await loadPhase2Plan({});
  ...
  assert.deepEqual(Object.keys(plan.requirements).sort(), ["COLLAB-01", "SEC-01"]);
  assert.equal(Object.keys(plan.roadmap).length, 5);
  assert.deepEqual(Object.keys(plan.assumptions).sort(), ["A1", "A2", "A3", "A4", "A5", "A6"]);
