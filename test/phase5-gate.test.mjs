/**
 * The Phase-5 gate must fail closed.
 *
...
function healthyPlan() {
  const commands = [
    ...
    { command: "npm run test:phase5:release", id: "T5-14", name: "Exact artifacts", owner: true },
  ];
  const mapping = (evidence) => ({ evidence, text: "seeded" });
  return {
    assumptions: { A1: mapping(["T5-14"]) },
    commands,
    plans: { "02-17": mapping(["T5-14"]) },
    requirements: { "SEC-01": mapping(["F2-02", "T5-14"]) },
    roadmap: { "RC2-1": mapping(["F2-02"]) },
    threats: { "T5-14": mapping(["T5-14"]) },
  };
}
...
function healthyResults() {
  return {
    ...
    "T5-14": {
      command: "npm run test:phase5:release",
      exit_code: 0,
...
test("a healthy graph reaches the T5-14 leaf exactly once", () => {
...
test("a missing leaf leaves T5-14 unowned", () => {
...
test("complete current evidence verifies", () => {
  assert.deepEqual(validatePhase5Evidence(healthyPlan(), healthyResults()), { verified: true });
});

test("missing evidence fails closed", () => {
  const results = healthyResults();
  delete results["T5-14"];
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /missing command result: T5-14/);
});

test("a failed command fails closed", () => {
  const results = healthyResults();
  results["T5-14"] = { ...results["T5-14"], exit_code: 1, passed: false };
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /command failed: T5-14/);
});

test("a skipped command fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], skipped: true };
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /skipped: F2-02/);
});

test("a run that asserted nothing fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], test_count: 0 };
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /zero tests recorded: F2-02/);
});

test("evidence recorded for a different command is stale", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], command: "npm test -- --only=fast" };
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /stale command result: F2-02/);
});

test("evidence with no output hash fails closed", () => {
  const results = healthyResults();
  results["F2-02"] = { ...results["F2-02"], output_sha256: "not-a-hash" };
  assert.throws(() => validatePhase5Evidence(healthyPlan(), results), /output hash missing: F2-02/);
});
...
test("a mapping that points at evidence nobody runs fails closed", () => {
  ...
  assert.throws(() => validatePhase5Evidence(plan, healthyResults()), /references missing evidence T2-99/);
});
...
test("an unmapped requirement, plan, roadmap truth or assumption fails closed", () => {
  for (const kind of ["assumptions", "plans", "requirements", "roadmap"]) {
    ...
    assert.throws(() => validatePhase5Evidence(plan, healthyResults()), new RegExp(`${id} .* is unmapped`), kind);
  }
});
...
test("a command nothing maps to fails closed", () => {
  ...
  assert.throws(() => validatePhase5Evidence(plan, results), /command evidence is unmapped: F2-99/);
});
...
test("the repository plan binds every threat, plan, requirement and assumption", async () => {
  const plan = await loadPhase5Plan({});
  assert.equal(plan.owner_commands.length, 16);
  assert.equal(Object.keys(plan.threats).length, 16);
  assert.equal(Object.keys(plan.plans).length, 20);
  assert.deepEqual(Object.keys(plan.requirements).sort(),
    ["CAD-01", "CAT-01", "ENG-01", "ENG-02", "SDK-01"]);
  assert.equal(Object.keys(plan.roadmap).length, 5);
  assert.deepEqual(Object.keys(plan.assumptions).sort(), [
    "BOTH_AXES_DISTINCT", "CONTRIBUTIONS_ARE_DATA", "COUNT_BY_RENDERING",
    "ENDPOINT_IS_A_PAIR", "FAILURE_IS_EXPLICIT", "REFUSAL_IS_EXPLAINED",
  ]);
  // The two bugs generating this gate from Phase 4's introduces, checked
  // rather than trusted: a roadmap slice whose bounds collapse onto the same
  // heading yields an empty block, and a plan regex still matching `04-`
  // yields no plans. Both would leave a gate that binds nothing and passes.
  assert.match(plan.roadmap["RC5-1"].text, /symbol variants/);
  assert.match(plan.plans["05-20"].text, /gate/);
