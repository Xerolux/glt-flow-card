 ...
 * mutation tests in test/phase9-gate.test.mjs check each derivation.
 */
...
export const PHASE_DIR = `.planning/phases/${PHASE_SLUG}-multi-site-supervision`;
...
const REQUIREMENT_EVIDENCE = {
  "SITE-01": [
    "T9-01", "T9-02", "T9-03", "T9-04", "T9-05", "T9-06", "T9-07",
    "T9-08", "T9-09", "T9-10", "T9-11", "T9-12", "T9-13", "T9-14",
    "T9-15", "T9-16", "T9-17", "T9-18", "T9-19", "T9-20",
  ],
};

/** Roadmap success criteria for Phase 9, in the order the roadmap states them. */
const ROADMAP_EVIDENCE = {
  "RC9-1": ["T9-03", "T9-04", "T9-05", "T9-07"],
  "RC9-2": ["T9-01", "T9-02", "T9-13", "T9-15", "T9-16", "T9-17"],
  "RC9-3": ["T9-08", "T9-09", "T9-11", "T9-12"],
  "RC9-4": ["T9-10", "T9-14", "T9-18", "T9-19"],
  "RC9-5": ["T9-02", "T9-06", "T9-15", "T9-16", "T9-20"],
};

/** Every plan in the phase, bound to the evidence its work is proven by. */
const PLAN_EVIDENCE = {
  "09-01": [`F${PHASE}-02`, `F${PHASE}-03`],
  "09-02": ["T9-13"],
  "09-03": ["T9-03", "T9-04", "T9-05"],
  "09-04": ["T9-13", "T9-15"],
  "09-05": ["T9-01", "T9-02", "T9-17"],
  "09-06": ["T9-06", "T9-07"],
  "09-07": ["T9-08", "T9-09", "T9-10"],
  "09-08": ["T9-11", "T9-12"],
  "09-09": ["T9-14"],
  "09-10": ["T9-16"],
  "09-11": ["T9-18", "T9-19"],
  "09-12": ["T9-20", `F${PHASE}-01`, `F${PHASE}-06`],
};
...
const ASSUMPTION_EVIDENCE = {
  DESTINATIONS_ARE_SERVER_OWNED: ["T9-03"],
  THE_ADDRESS_IS_CHECKED_NOT_THE_NAME: ["T9-04"],
  CREDENTIALS_NEVER_LEAVE: ["T9-07"],
  ONE_REQUEST_PER_SITE: ["T9-01"],
  THE_DEADLINE_OWNS_THE_REQUEST: ["T9-02"],
  ABSENT_IS_NEVER_UNAVAILABLE: ["T9-13"],
  PARTIAL_IS_AN_ANSWER: ["T9-14"],
  REMOTE_REUSES_LOCAL_AUTHORITY: ["T9-09"],
  A_TIMEOUT_IS_NOT_A_FAILURE: ["T9-12"],
};

const ASSUMPTION_TEXTS = {
  DESTINATIONS_ARE_SERVER_OWNED: "A remote destination is site configuration on a server-owned allowlist; a project document can neither add one nor reach one that is not listed.",
  THE_ADDRESS_IS_CHECKED_NOT_THE_NAME: "The resolved address is checked at connection time against loopback, link-local, private and unique-local ranges, so an allowlist survives a name that resolves privately later.",
  CREDENTIALS_NEVER_LEAVE: "A site token appears in no response, log line, export or error message, and a seeded sentinel is searched for across every output of every remote path including its failure branches.",
  ONE_REQUEST_PER_SITE: "Remote state is read with one request per site rather than one per entity, and sites are asked concurrently up to a declared bound.",
  THE_DEADLINE_OWNS_THE_REQUEST: "A total deadline belongs to the request and is not divisible among sites; a request that hits it returns what it has with the rest stated absent.",
  ABSENT_IS_NEVER_UNAVAILABLE: "Unreachability belongs to the site and is never written as an entity state, so an entity we could not ask about stays distinguishable from one that is genuinely unavailable.",
  PARTIAL_IS_AN_ANSWER: "Every roll-up states which sites answered, which did not and why, and every aggregate states its own completeness.",
  REMOTE_REUSES_LOCAL_AUTHORITY: "Every remote route enforces the same capability and project scoping as its local equivalent, and a listing is filtered before it is limited.",
  A_TIMEOUT_IS_NOT_A_FAILURE: "A remote timeout is effect_unknown rather than failed, and no surface offers a retry beside one.",
};
