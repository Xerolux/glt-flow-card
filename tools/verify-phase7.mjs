 ...
 * mutation tests in test/phase7-gate.test.mjs check each derivation.
 */
...
export const PHASE_DIR = `.planning/phases/${PHASE_SLUG}-trends-energy-reports`;
...
const FOCUSED_COMMANDS = [
  ["F7-01", "Canonical build from authored modules", "npm run build"],
  ["F7-02", "Node regression suites", "npm test"],
  ["F7-03", "Companion suite", "npm run test:python"],
  ["F7-04", "Exact-dist browser suites", "npm run test:e2e"],
  ["F7-05", "Complete sources and deterministic documentation site", "node tools/verify-docs-site.mjs"],
  // The previous phase gate stays mandatory: a Phase-7 claim resting on Phase-6
  // guarantees nobody re-ran is resting on nothing.
  ["F7-06", `Phase-${PHASE - 1} gate`, `npm run test:phase${PHASE - 1}`],
...
const REQUIREMENT_EVIDENCE = {
  "HIST-01": [
    "T7-01", "T7-02", "T7-03", "T7-04", "T7-05",
    "T7-06", "T7-07", "T7-08", "T7-19", "T7-20", "T7-21", "T7-22", "T7-23",
  ],
  "ENER-01": ["T7-09", "T7-10", "T7-11", "T7-12", "T7-13"],
  "REPORT-01": ["T7-14", "T7-15", "T7-16", "T7-17", "T7-18"],
};

/** Roadmap success criteria for Phase 7, in the order the roadmap states them. */
const ROADMAP_EVIDENCE = {
  "RC7-1": ["T7-01", "T7-02", "T7-03", "T7-04", "T7-05"],
  "RC7-2": ["T7-07", "T7-08", "T7-09", "T7-10", "T7-11", "T7-13"],
  "RC7-3": ["T7-14", "T7-15", "T7-16", "T7-17"],
  "RC7-4": ["T7-06", "T7-12", "T7-18"],
  "RC7-5": ["T7-19", "T7-20", "T7-21", "T7-22", "T7-23"],
};
...
const PLAN_EVIDENCE = {
  ...
  "07-02": ["T7-03", "T7-21"],
  "07-03": ["T7-21"],
  "07-04": ["T7-01", "T7-02", "T7-03", "T7-06"],
  "07-05": ["T7-07", "T7-08", "T7-09", "T7-14", "T7-16"],
  "07-06": ["T7-07"],
  "07-07": ["T7-08"],
  "07-08": ["T7-01"],
  "07-09": ["T7-02"],
  "07-10": ["T7-03", "T7-04", "T7-05"],
  "07-11": ["T7-06"],
  "07-12": ["T7-09", "T7-10"],
  "07-13": ["T7-11", "T7-12", "T7-13"],
  "07-14": ["T7-14", "T7-18"],
  "07-15": ["T7-15"],
  "07-16": ["T7-16", "T7-17"],
  "07-17": ["T7-18"],
...
const ASSUMPTION_EVIDENCE = {
  SERVER_OWNS_HISTORY: ["T7-01"],
  QUERIES_ARE_BOUNDED: ["T7-02"],
  COVERAGE_TRAVELS: ["T7-03"],
  ABSENT_IS_NEVER_A_VALUE: ["T7-04"],
  PERIODS_ARE_DELEGATED: ["T7-07"],
  TWO_METER_MODELS: ["T7-10"],
  UNITS_ARE_CHECKED: ["T7-11"],
  RUNS_ARE_REPRODUCIBLE: ["T7-14"],
  REACHABLE_IS_NOT_REACHED: ["T7-22"],
};

const ASSUMPTION_TEXTS = {
  SERVER_OWNS_HISTORY: "Every history read goes through a declared Companion route, enforced server-side and audited; the browser issues no Recorder request of its own.",
  QUERIES_ARE_BOUNDED: "Rows, entities and window are each a configured bound enforced before the query, and a request past a bound is refused with the limit named or answered from statistics and labelled -- never truncated silently.",
  COVERAGE_TRAVELS: "Every series, period total and report figure carries a coverage fraction and a gap list as fields of the result, so ignoring them has to be deliberate.",
  ABSENT_IS_NEVER_A_VALUE: "A missing sample is never zero, never the nearest neighbour and never interpolated across; the renderer breaks the line rather than crossing it.",
  PERIODS_ARE_DELEGATED: "A named period is resolved by Home Assistant on local-calendar boundaries in the site timezone; our code names, validates and renders a period but never computes one.",
  TWO_METER_MODELS: "A counter is differenced across the resolved period and a rate integrated over it, and the two models are never converted into each other implicitly.",
  UNITS_ARE_CHECKED: "Units are validated against the meter, the model and the price before any arithmetic, and an incompatible pair is refused with a reason rather than converted on a guess.",
  RUNS_ARE_REPRODUCIBLE: "A report run records window, timezone, aggregate, deadband, sources and coverage, and re-running reproduces the value or names which input changed.",
  REACHABLE_IS_NOT_REACHED: "Every retired evaluator is proven by rendering the surface and reading the number, never by a grep over the artifact alone.",
};
 ...
 * every row must carry an owner command. A register missing T7-07 entirely
