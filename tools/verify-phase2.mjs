/* Execute and hash the complete Phase-2 behavioral evidence set.
 *
 * The gate answers one question: is every Phase-2 requirement, roadmap truth,
 ...
 *
 * It also proves its own shape. The command graph must be acyclic, the T2-16
...
const PHASE_DIR = ".planning/phases/02-authoritative-policy-controls-collaboration";
const DEFAULT_OUTPUT = ".planning/tmp/phase2-evidence.json";

/** The only outer entry point, its tool, and the one T2-16 leaf. */
export const OUTER_SCRIPT = "test:phase2";
export const ORCHESTRATOR_TOOL = "tools/verify-phase2.mjs";
export const LEAF_SCRIPT = "test:phase2:release";

/** Commands that are not a threat owner but must still be current and green. */
const FOCUSED_COMMANDS = [
  ["F2-01", "Canonical build from authored modules", "npm run build"],
...
const REQUIREMENT_EVIDENCE = {
  "SEC-01": ["T2-01", "T2-02", "T2-03", "T2-04", "T2-05", "T2-06", "T2-07", "T2-08", "T2-09", "T2-13", "T2-14", "T2-16"],
  "COLLAB-01": ["T2-10", "T2-11", "T2-12", "T2-13", "T2-14", "T2-15", "T2-16"],
};

/** Roadmap success criteria for Phase 2, in the order the roadmap states them. */
const ROADMAP_EVIDENCE = {
  "RC2-1": ["T2-01", "T2-02", "T2-03", "T2-04", "T2-05"],
  "RC2-2": ["T2-06", "T2-07", "T2-08"],
  "RC2-3": ["T2-04", "T2-08", "T2-09"],
  "RC2-4": ["T2-10", "T2-11", "T2-12", "T2-14"],
  "RC2-5": ["T2-13", "T2-14", "T2-15"],
};
...
const PLAN_EVIDENCE = {
  ...
  "02-04": ["T2-06", "T2-15"],
  "02-05": ["T2-13", "T2-14"],
  "02-06": ["T2-01", "T2-02", "T2-03"],
  "02-07": ["T2-04", "T2-05"],
  "02-08": ["T2-10"],
  "02-09": ["T2-11", "T2-12"],
  "02-10": ["T2-09"],
  "02-11": ["T2-06", "T2-07", "T2-08"],
  "02-12": ["T2-13"],
  "02-13": ["T2-12", "T2-14"],
  "02-14": ["T2-15"],
  "02-15": ["T2-16", "F2-01"],
...
const ASSUMPTION_TEXTS = {
  A1: "Exact byte, depth, node, key, rate and TTL bounds are declared once and enforced.",
  A2: "A Home Assistant administrator holds membership repair only, never content authority.",
  A3: "Content revisions and access revisions are separate streams.",
  A4: "An operator runs a configured control without holding the engineering lease.",
  A5: "Evidence pages use short-lived server-state cursors with no exposed total.",
  A6: "Remote routes are declared and fail closed until Phase 9.",
};
 ...
 * loops forever, a missing leaf means T2-16 is unowned, more than one path
...
export function assertCommandGraph(graph) {
  ...
  if (!graph.has(leaf)) throw new Error(`the T2-16 leaf ${LEAF_SCRIPT} is not defined`);
...
export async function loadPhase2Plan(options = {}) {
  ...
  if (leafOwners.length !== 1 || leafOwners[0].id !== "T2-16") {
    throw new Error(`${LEAF_SCRIPT} must be the owner command of T2-16 alone`);
  }
  ...
  // no row invents one, and T2-16 is owned by the leaf in both documents.
  const validationRows = tableRows(validationMarkdown)
    .filter((cells) => cells.length === 6 && /^T2-\d{2}/.test(cells[1]));
  ...
  for (const cells of validationRows) {
    ...
    for (const threat of cells[1].split(",").map((value) => value.trim())) {
      ...
      if (threat === "T2-16" && command !== `npm run ${LEAF_SCRIPT}`) {
        throw new Error(`the validation map must own T2-16 with npm run ${LEAF_SCRIPT}`);
      }
  ...
  const planTexts = Object.fromEntries([...roadmapBlock.matchAll(/^- \[[ x]\] (02-\d{2})-PLAN\.md — (.+)$/gm)]
  ...
  return {
    ...
    requirements: evidenceMap({
      "SEC-01": "Every shared read, mutation, subscription, control and audit decision is server-authorized",
      "COLLAB-01": "Concurrent engineers cannot overwrite one another",
...
export function validatePhase2Evidence(plan, results) {
...
async function artifactIdentities(root) {
  const files = [
    ...
    ".planning/ROADMAP.md",
    `${PHASE_DIR}/02-THREATS.md`,
    `${PHASE_DIR}/02-VALIDATION.md`,
    "README.de.md",
    "README.md",
...
function parseArgs(argv) {
  ...
  for (const argument of argv) {
    ...
    else throw new Error(`unknown Phase-2 gate argument: ${argument}`);
  }
  return { output };
}
...
async function main() {
  ...
  const plan = await loadPhase2Plan({ root: ROOT });
  ...
  validatePhase2Evidence(plan, results);
  ...
  const report = canonical({
    ...
    commands: results,
    format: "glt-flow-card-phase2-evidence",
  ...
  console.log(`PASS Phase-2 evidence ${path.relative(ROOT, outputPath)}`);
  console.log(`PASS ${Object.keys(results).length} unique commands including all sixteen threat owners exactly once`);
}
