...
(() => {
  ...
  function addonToolbar(editor){const bar=editor.shadowRoot.querySelector(".glt-v1-toolbar");if(!bar)return;if(!bar.querySelector("[data-zup]")){const up=document.createElement("button"),down=document.createElement("button"),rep=document.createElement("button");up.dataset.zup="1";up.textContent="↑ Z";down.dataset.zdown="1";down.textContent="↓ Z";rep.dataset.report="1";rep.textContent="▤ Reports";up.onclick=()=>reorder(editor,1);down.onclick=()=>reorder(editor,-1);rep.onclick=()=>reportPanel(editor);bar.append(up,down,rep);}lasso(editor);}
