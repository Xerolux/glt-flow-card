...
(() => {
  ...
  function box(owner,title,html){const m=window.GLTFlowCardSDK.openDialog(owner,title,html);m.classList.add("glt-v1-addon-modal");return m;}
  ...
  function addRuntimeButtons(card){const root=card.shadowRoot;const group=root.querySelector("[data-menu-group='operation']");const bar=group||root.querySelector(".glt4-tool,.glt-toolbar,.toolbar,.glt-head-actions");if(!bar||bar.querySelector("[data-v1-energy]"))return;const b=document.createElement("button");b.className=group?"glt-menu-item":"glt4-pill glt-v1-btn";if(group){b.setAttribute("role","menuitem");b.innerHTML=`<ha-icon icon="mdi:lightning-bolt-circle"></ha-icon>${esc(gltText("legacy.button_energy"))}`;}else{b.textContent=gltText("legacy.button_energy");}b.dataset.v1Energy="1";b.onclick=()=>energyPanel(card);bar.appendChild(b);}
