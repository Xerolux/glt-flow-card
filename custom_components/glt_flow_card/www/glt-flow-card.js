...
(() => {
  ...
  var P = (id, label, category, type, ports = [], slots = [], controls = []) => ({
    id,
    label,
    category,
    type,
  ...
  var port = (id, medium, side, direction = "bidirectional") => ({ id, medium, side, direction });
  var slot = (id, label, domains = ["sensor"], unit = null) => ({ id, label, domains, unit });
  var control = (id, label, command, domains = []) => ({ id, label, command, domains });
  ...
  var BASE_SYMBOLS = [
    ...
    ["pump_dhw", "Zirkulationspumpe", "Hydraulik", "pump"],
    ["valve_2way", "2-Wege-Ventil", "Hydraulik", "valve"],
  ...
  function profileForEquipment(item = {}) {
  ...
  function portsForEquipment(item = {}) {
  ...
  function ensureV1(raw = {}) {
    ...
    c.historian = { aggregate: "raw", deadband: 0, max_points: 4e3, ...c.historian || {} };
  ...
  function testSignal(states, spec, nowMs = Date.now()) {
    ...
    if (typeof spec === "string") {
      const s2 = stateObj(states, spec);
      return s2 ? isActiveValue(s2.state) : false;
    }
    const s = stateObj(states, spec.entity);
    if (!s) return spec.missing_is_active === true;
    if (spec.attribute) {
      const value = s.attributes?.[spec.attribute];
      if (spec.equals !== void 0) return String(value) === String(spec.equals);
      if (spec.not_equals !== void 0) return String(value) !== String(spec.not_equals);
    ...
    if (spec.max_age_minutes) {
      ...
      return Number.isFinite(t) && nowMs - t > Number(spec.max_age_minutes) * 6e4;
    }
    return isActiveValue(s.state);
  }
  function deriveOperationalState(item, states, options = {}) {
    ...
    if (primary) {
      ...
      const updated2 = Date.parse(primary.last_updated || primary.last_changed || "");
      add("stale", Number.isFinite(updated2) && nowMs - updated2 > staleMinutes * 6e4);
  ...
  var words = (s) => new Set(normalizeText(s).split(/\s+/).filter(Boolean));
  function scoreEntityForSlot(entityId, state, slotSpec, equipment = {}) {
  ...
  function autoMapEquipment(equipment, hassStates = {}) {
  ...
  function aggregateSeries(points, options = {}) {
    ...
    if (deadband > 0 && src.length) {
      const out = [src[0]];
      for (const p of src.slice(1)) if (Math.abs(p.y - out.at(-1).y) >= deadband) out.push(p);
      src = out;
    }
  ...
  (() => {
    ...
    const currentRole = (c, hass) => {
      ...
      if (id && c?.permissions?.designers?.includes(id)) return "designer";
      if (id && c?.permissions?.operators?.includes(id)) return "operator";
      return "viewer";
    };
    ...
    sdk.registerPanel = (p) => sdk.panels.set(p.id, p);
    sdk.registerMigration = (m) => sdk.migrations.push(m);
    ...
    sdk.projectDiff = projectDiff;
    sdk.makeProjectBundle = makeProjectBundle;
    sdk.readProjectBundle = readProjectBundle;
    window.GLTFlowCardSDK = sdk;
    const STYLES = `
  ...
  body.glt-v1-kiosk .header,body.glt-v1-kiosk app-toolbar{display:none!important}@media(min-width:1800px){.glt-v1-dialog{width:min(1320px,96vw)}}`;
    ...
    function editorModal(editor, title, html) {
      return modal(editor, title, html);
    }
    ...
    function selectedRefs(editor) {
      ...
      return editor._sel ? [{ kind: editor._sel.k, id: editor._sel.id }] : [];
    }
    ...
    function showSymbolLibrary(editor) {
      ...
      const render = () => {
        ...
        m.querySelectorAll("[data-use]").forEach((b) => b.onclick = () => {
          ...
          current.profile = s.profile;
          editor._config.appearance = editor._config.appearance || {};
          editor._config.appearance.mode = s.style;
          emit(editor);
          m.remove();
        });
      };
      m.querySelector("[data-q]").oninput = render;
      m.querySelector("[data-style]").onchange = render;
      render();
    }
    ...
    function showCAD(editor) {
      ...
      m.querySelector("[data-paste]").onclick = () => {
        ...
        for (const c of editor._gltV1Clipboard || []) {
          ...
          (c.kind === "equipment" ? cfg.equipment : c.kind === "datapoint" ? cfg.datapoints : c.kind === "path" ? cfg.paths : cfg.kpis).push(o);
        }
        emit(editor);
        m.remove();
      };
      m.querySelector("[data-layer]").onclick = () => {
        m.remove();
        showLayers(editor);
      };
    }
    function showLayers(editor) {
      ...
      m.querySelector("[data-save]").onclick = () => {
        m.querySelectorAll("[data-l]").forEach((r) => {
          const l = cfg.layers[+r.dataset.l];
          l.name = r.querySelector("[data-name]").value;
          l.visible = r.querySelector("[data-vis]").checked;
          l.locked = r.querySelector("[data-lock]").checked;
        });
        emit(editor);
        m.remove();
      };
    ...
    function showSimulation(editor) {
      ...
      m.querySelector("[data-save]").onclick = () => {
        ...
        if (item) {
          cfg.simulation.states[item.id] = { state: m.querySelector("[data-state]").value, value: m.querySelector("[data-value]").value };
        }
        emit(editor);
        m.remove();
      };
    ...
    function showEnergy(editor) {
      ...
      m.querySelector("[data-save]").onclick = () => {
        m.querySelectorAll("[data-e]").forEach((r, i) => Object.assign(cfg.energy.meters[i], { name: r.querySelector("[data-name]").value, kind: r.querySelector("[data-kind]").value, entity: r.querySelector("[data-entity]").value, price_per_unit: Number(r.querySelector("[data-price]").value) || void 0 }));
        emit(editor);
        m.remove();
      };
    }
    ...
    function applyLayers(editor) {
      const cfg = editor._config, layers = new Map(cfg.layers.map((l) => [l.id, l]));
    ...
    function minimap(editor) {
      const root = editor.shadowRoot;
      if (editor._config.ui?.minimap === false) {
        root.querySelector(".glt-v1-minimap")?.remove();
        return;
      }
      const host = root.querySelector(".canvas,.stage,[data-canvas],.draw");
      if (!host || root.querySelector(".glt-v1-minimap")) return;
      const m = document.createElement("div");
      m.className = "glt-v1-minimap";
      const cw = editor._config.canvas?.width || 1600, ch = editor._config.canvas?.height || 900;
      for (const e of editor._config.equipment || []) {
        const d = document.createElement("div");
        d.className = "glt-v1-miniitem";
        d.style.left = `${(e.x || 0) / cw * 180}px`;
        d.style.top = `${(e.y || 0) / ch * 110}px`;
        d.style.width = `${Math.max(3, (e.width || 180) / cw * 180)}px`;
        d.style.height = `${Math.max(3, (e.height || 100) / ch * 110)}px`;
        m.appendChild(d);
      }
    ...
    function editorToolbar(editor) {
      ...
      const base = root.querySelector(".glt4-bar,.toolbar,.bar,.tb") || root.firstElementChild;
      if (!base) return;
      const bar = document.createElement("div");
  ...
  (() => {
    ...
    const prevRE = Card.prototype._renderEquipment;
    Card.prototype._renderEquipment = function(canvas) {
      const r = prevRE.call(this, canvas);
      addDrilldown(this, canvas);
      return r;
    };
    const prevR = Card.prototype._render;
    Card.prototype._render = function() {
      const r = prevR.call(this);
      addRuntimeButtons(this);
      breadcrumbs(this);
      return r;
    };
    function reorder(editor, dir) {
      ...
      if (!ids.size) return;
      const a = editor._config.equipment;
      if (dir > 0) {
        for (let i = a.length - 2; i >= 0; i--) if (ids.has(a[i].id) && !ids.has(a[i + 1].id)) [a[i], a[i + 1]] = [a[i + 1], a[i]];
      } else {
        for (let i = 1; i < a.length; i++) if (ids.has(a[i].id) && !ids.has(a[i - 1].id)) [a[i], a[i - 1]] = [a[i - 1], a[i]];
      }
      editor._emit?.();
      editor._render?.();
    }
    function lasso(editor) {
      const root = editor.shadowRoot, stage = root.querySelector(".stage,.canvas,.draw,[data-canvas]");
      if (!stage || stage.dataset.v1Lasso) return;
      stage.dataset.v1Lasso = "1";
      stage.addEventListener("pointerdown", (e) => {
        if (!e.shiftKey || e.button !== 0 || e.target.closest?.("[data-k],.glt-equipment")) return;
        const sr = stage.getBoundingClientRect(), sx = e.clientX, sy = e.clientY;
        ...
        const up = (ev) => {
          ...
          rect.remove();
          editor._glt4Multi = editor._glt4Multi || /* @__PURE__ */ new Set();
          if (!e.ctrlKey && !e.metaKey) editor._glt4Multi.clear();
          root.querySelectorAll('[data-k="equipment"][data-id],.glt-equipment[data-id]').forEach((n) => {
            const r = n.getBoundingClientRect();
            if (r.right >= sel.left && r.left <= sel.right && r.bottom >= sel.top && r.top <= sel.bottom) editor._glt4Multi.add(`equipment:${n.dataset.id}`);
          });
          editor._render?.();
        };
    ...
    function addonToolbar(editor) {
      const bar = editor.shadowRoot.querySelector(".glt-v1-toolbar");
      if (!bar) return;
